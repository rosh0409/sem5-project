...
const Home = () => {
  ...
  const handleCart = async(e,index) =>{
    const res = await axios.post("http://localhost:8000/api/add-to-cart",{"index":index-1},{withCredentials:true})
    if(res.data.status === "success"){
              ...
              return(
                ...
                <CardContent>
                  ...
                  <Stack gap={2}>
                    ...
                    <Button color="secondary" variant="outlined" onClick={(e)=>handleCart(e,index)}>add to cart</Button>
                  </Stack>
                </CardContent>
              </Card>
              </Box>
